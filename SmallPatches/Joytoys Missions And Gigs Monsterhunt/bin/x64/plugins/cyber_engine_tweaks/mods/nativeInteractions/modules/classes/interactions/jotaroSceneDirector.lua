-- Joytoys Missions and Gigs: Monsterhunt - embedded two-actor scene director
--
...
local function normalizeScene(definition)
    ...
    local partnerProfile = definition.partnerProfile or "tyger"
    if not PARTNER_PROFILES[partnerProfile] then partnerProfile = "tyger" end
    return {
...
local function createDirector(logger)
    ...
    local function initialize()
        ...
        local assembleOK = pcall(function()
            Observe("JJR_EmbeddedSceneEntityService", "OnAssemble", function(_, event)
                dispatchCallback(director.assembleCallbacks, event)
            end)
        end)
        local attachOK = pcall(function()
            Observe("JJR_EmbeddedSceneEntityService", "OnAttached", function(_, event)
    ...
    local function partnerOnlyMoaning(active)
        ...
        return poseName == "oral01" or poseName == "oral05"
    end
    ...
    local function spawnActor(active, role, record, appearance, hiddenOffset)
        ...
        spec.tags = {
            CName.new(role == "player" and "JJRScenePlayer" or "JJRScenePartner"),
            CName.new("JotaroJoytoyRoute")
        }
    ...
    function director.preflight(anchor, definition)
        ...
        if type(anchor) ~= "table" or type(anchor.position) ~= "table" or type(anchor.rotation) ~= "table" then
            return false, "Jotaro bedroom anchor is incomplete"
        end
        ...
        if not resourceExists(profile.template) then
            return false, "Lizzie's BDs nude " .. profile.label .. " template is unavailable"
        end
        if not resourceExists(PARTNER_ERECT_MESH) then
        ...
        if not ensurePartnerRecord(profile) then
            return false, "route-owned nude " .. profile.label .. " character record could not be created"
        end
