...
logic = {
        ...
        yaw = 0,
        lastStatusCheck = 0  -- Track last time we validated status effects
}
...
function logic.forceCleanupFlightEffects()
        -- Force removal of all flight-related status effects
        -- Called when flight state becomes inconsistent with active effects
        print("[FreeFly] DEBUG: FORCED CLEANUP of flight effects initiated")
        ...
        print("[FreeFly] DEBUG: FORCED CLEANUP completed")
end

function logic.toggleFlight(mod, state)
        if state then
            print("[FreeFly] DEBUG: Flight ACTIVATED - applying effects")
        else
            print("[FreeFly] DEBUG: Flight DEACTIVATED - removing effects")
        end
        ...
        if state then
            ...
            print("[FreeFly] DEBUG: Flight effects applied (NoWeapon: " .. tostring(mod.settings.noWeapon) .. ")")
        elseif not state then
            miscUtil.removeStatus("GameplayRestriction.NoZooming")
            miscUtil.removeStatus("GameplayRestriction.NoMovement")
            Game.GetTimeSystem():UnsetTimeDilation("console")
            print("[FreeFly] DEBUG: Flight effects removed")
        end
end
