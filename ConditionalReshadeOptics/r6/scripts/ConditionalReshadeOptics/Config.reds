public class ConditionalReshadeOpticsConfig {
    ...
    @runtimeProperty("ModSettings.mod", "CONDITIONAL RESHADE OPTICS")
    @runtimeProperty("ModSettings.category", "Dynamic effects")
    @runtimeProperty("ModSettings.category.order", "15")
    @runtimeProperty("ModSettings.displayName", "Enable dynamic effects")
    @runtimeProperty("ModSettings.description", "If ON, enables dynamic ReShade technique switching based on status effects (exhausted, bleeding, burning, etc.).")
    let effectDynamicStatusON: Bool = true;

    @runtimeProperty("ModSettings.mod", "CONDITIONAL RESHADE OPTICS")
    @runtimeProperty("ModSettings.category", "Dynamic effects")
    @runtimeProperty("ModSettings.category.order", "16")
    @runtimeProperty("ModSettings.displayName", "Enable cinematic immunity effect")
    @runtimeProperty("ModSettings.description", "If ON, enables the Border ReShade technique when cinematic immunity is active.")
    let effectCinematicImmunityON: Bool = true;
