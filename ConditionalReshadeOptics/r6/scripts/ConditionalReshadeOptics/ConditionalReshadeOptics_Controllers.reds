...
@wrapMethod(RadialMenuHubGameController)
  ...
  protected cb func OnOpenMenuRequest(evt: ref<OpenMenuRequest>) -> Bool {
    ...
    if !IsDefined(_playerPuppetPS.m_conditionalOptics) {
        ...
        _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] OnOpenMenuRequest: runtime not available yet.");
    } else {
        _playerPuppetPS.m_conditionalOptics.refresh();
        _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] OnOpenMenuRequest: runtime available, refreshed profile.");
    }
...
@wrapMethod(scannerGameController)
  ...
  private final func ShowScanner(show: Bool) -> Void {
    ...
    // _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] ScannerGameController: START SCANNING");

    if !IsDefined(_playerPuppetPS.m_conditionalOptics) {
        _playerPuppetPS.InitConditionalOpticsSystem(_playerPuppet);
        _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] ScannerGameController: runtime not available yet.");
    } else {
        _playerPuppetPS.m_conditionalOptics.refresh();
        // _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] ScannerGameController: runtime available, refreshed profile.");
    }
...
protected cb func OnEnterScenario(prevScenario: CName, userData: ref<IScriptable>) -> Bool {
  ...
  if IsDefined(_playerPuppet) {
    _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] Arcade started! Found player puppet" );
    _playerPuppetPS.m_conditionalOptics.isArcadeMachineON = true; 
    _playerPuppetPS.m_conditionalOptics.refreshReshadeProfile();
      
  } else {
    _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] Arcade started, but PlayerPuppet could not be found.");
  }
}
...
protected cb func OnArcadeMinigameEnd() -> Bool {
  ...
  if IsDefined(_playerPuppet) {  
    ...
    _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] Arcade ended! " );
    _playerPuppetPS.m_conditionalOptics.isArcadeMachineON = false;
    _playerPuppetPS.m_conditionalOptics.refreshReshadeProfile();
 
     
  } else {
    _playerPuppetPS.m_conditionalOptics.showDebugMessage("[ConditionalReshadeOptics] Arcade ended, but PlayerPuppet could not be found.");
  }
